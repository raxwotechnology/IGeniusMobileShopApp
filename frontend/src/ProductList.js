...
//const API_URL = 'https://igeniusmobileshopapp-xagk.onrender.com/api/products';
 const API_URL = 'https://igeniusmobileshopapp-xagk.onrender.com/api/products';
const CLICKED_PRODUCTS_API_URL = 'https://igeniusmobileshopapp-xagk.onrender.com/api/clicked-products';

const REPAIRS_API_URL = 'https://igeniusmobileshopapp-xagk.onrender.com/api/productsRepair';
const PAYMENTS_API_URL = 'https://igeniusmobileshopapp-xagk.onrender.com/api/payments';
const PAYMENTS_API_URLwithItemcode = 'https://igeniusmobileshopapp-xagk.onrender.com/api/payments/with-itemcodes';
...
const ProductList = ({ darkMode }) => {
  ...
  const [usedrQuantities, setUsedrQuantities] = useState({});
  ...
  const fetchProducts = () => {
    setLoading(true);
    setRefreshing(true);
    let url = `https://igeniusmobileshopapp-xagk.onrender.com/api/product-uploads`;
    fetch(url)
      .then((response) => {
        if (!response.ok) {
      ...
      .then((data) => {
        ...
        const rawProducts = Array.isArray(data.records4)
          ? data.records4
          : data && typeof data === "object" && "records4" in data
            ? data.records4
            : [];
        ...
        if (data && data.records) {
          ...
          setTotalProducts(data.total4 || availableProducts.length);
        } else if (Array.isArray(data)) {
          setProducts(data);
          setTotalPages(1);
          setTotalProducts(data.records4.length);
        } else {
          setProducts([]);
          setTotalPages(1);
          setTotalProducts(0);
  ...
  const loadAllUsage = async () => {
    ...
    try {
      ...
      repairsList.forEach(repair => {
        ...
        (repair.repairCart || []).forEach(item => {
          if (item.itemCode) {
            ...
            rusageMap[item.itemCode] = (rusageMap[item.itemCode] || 0) + (item.quantity || 0);
          }
        });
      ...
      allPayments.forEach(payment => {
        (payment.items || []).forEach(item => {
          if (item.itemCode) {
            ...
            pusageMap[item.itemCode] = (pusageMap[item.itemCode] || 0) + (item.quantity || 0) - (item.retquantity || 0);
          }
        });
      });
      ...
      setUsedpQuantities(pusageMap);
      setUsedrQuantities(rusageMap);
    } catch (err) {
  ...
  const loadSupplierQuantities = async () => {
    ...
    try {
      const res = await fetch('https://igeniusmobileshopapp-xagk.onrender.com/api/suppliers');
  ...
  const paginatedProductsForModal = sortedAndFilteredProducts.slice(
  ...
  const handleUploadExcel = async () => {
    ...
    try {
      ...
      const response = await fetch('https://igeniusmobileshopapp-xagk.onrender.com/api/product-uploads/bulk-upload',
      {
        method: 'POST',
  ...
  const fetchUploadedRecords = async (page = 1) => {
    ...
    setUploadedError('');
    try {
      const res = await fetch(`https://igeniusmobileshopapp-xagk.onrender.com/api/product-uploads?page=${page}&limit=20`);
  ...
  const handleExportAllUploaded = async () => {
    try {
      const res = await fetch('https://igeniusmobileshopapp-xagk.onrender.com/api/product-uploads/export-all');
                  ...
                  <tr key={product._id || product.itemCode || idx} style={product.source === 'uploaded' ? { background: '#f7f7f7' } : {}}>
                    ...
                    {/* <td>
                      {usageLoading ? (
                        ...
                        <span style={{ color: product.stock <= 2 ? (product.stock === 0 ? 'red' : '#2957F0') : 'black', fontWeight: 'bold' }}>
                          {usedrQuantities[product.itemCode]}
                        </span>
                      ) : (
                        '—'
                      )}
                    ...
                    <td>
                      {usageLoading ? (
                        <span>⋯</span>
                      ) : usedpQuantities[product.itemCode] !== undefined ? (
                        <span style={{ color: product.stock <= 2 ? (product.stock === 0 ? 'red' : '#2957F0') : 'black', fontWeight: 'bold' }}>
                          {usedpQuantities[product.itemCode]}
                        </span>
                      ) : (
                        '—'
                      )}

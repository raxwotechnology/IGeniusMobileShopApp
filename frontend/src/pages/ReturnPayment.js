...
const ReturnPayment = ({ onClose, darkMode, cashierId, cashierName }) => {
  ...
  useEffect(() => {
    ...
    axios.get('https://igeniusmobileshopapp-xagk.onrender.com/api/products', {
      headers: {
  ...
  const handleReturnPayment = async () => {
    ...
    try {
      const response = await fetch("https://igeniusmobileshopapp-xagk.onrender.com/api/payments/return", {
        method: "POST",
        headers: {

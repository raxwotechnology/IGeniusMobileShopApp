...
const CartDetailsTable = ({ supplierId, darkMode, refreshSuppliers }) => {
  ...
  return (
    <div className={`cart-details-container ${darkMode ? 'dark' : ''}`}>
      <h3 className={`cart-details-title ${darkMode ? 'dark' : ''}`}>{supplierName ? `${supplierName} Past Payments` : 'Past Payments'}</h3>
      ...
      {!loading && items.length === 0 ? (
        <p className="no-products">No Past Payments.</p>
      ) : (

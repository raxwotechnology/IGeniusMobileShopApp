...
const PaymentForm = ({ supplier, closeModal, fetchGrnReturnStocks, refreshSuppliers, darkMode }) => {
  ...
  useEffect(() => {
    const fetchReturnedValue = async () => {
      try {
        const response = await fetch(`https://igeniusmobileshopapp-5oi6.onrender.com/api/suppliers/retitems/${encodeURIComponent(supplier.supplierName)}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      for (const pay of validPayments) {
        const response = await fetch(`https://igeniusmobileshopapp-5oi6.onrender.com/api/suppliers/${supplier._id}/payments`, {
          method: 'POST',
          headers: { 
